...
export function startLiveAircraft(terria, adapterUrl, pollIntervalMs = 5000) {
  const aircraft = new Map(); // icao24 -> { billboard, lat, lon, alt, velocity, heading }
  let billboards = null;
  let lastFrameTime = 0;

  function tryAttach() {
    const cesium = terria.cesium;
    if (cesium && cesium.scene) {
      billboards = cesium.scene.primitives.add(
        new BillboardCollection({ scene: cesium.scene })
      );
      poll();
      setInterval(poll, pollIntervalMs);
      requestAnimationFrame(animate);
      console.log("[LiveAircraft] started with BillboardCollection");
    } else {
  ...
  async function poll() {
    ...
    try {
      ...
      for (const row of data.rows) {
        ...
        if (existing) {
          // Update existing — just change target data, animation loop moves it
          ...
          if (row.true_track !== null) {
            existing.billboard.alignedAxis = headingToEcefUnit(
              row.latitude,
              row.longitude,
              row.true_track
            );
      ...
      for (const [icao24, ac] of aircraft) {
        if (!seen.has(icao24)) {
          billboards.remove(ac.billboard);
    ...
    if (dtMs > 0 && dtMs < 1000) {
      ...
      for (const ac of aircraft.values()) {
        if (ac.velocity > 0 && ac.heading !== null) {
          ...
          ac.lat = moved.lat;
          ac.lon = moved.lon;
          ac.billboard.position = Cartesian3.fromDegrees(
            moved.lon,
            moved.lat,
            ac.alt
          );
